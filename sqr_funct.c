...
int solve(Coefficient *coef, Answer *ans) // pass coef as a pointer
{
    ...
    else
    {
        ...
        else
        {
            ...
            ans->ad1 = ( (-1) * coef->b - sqr_discr ) / (2 * coef->a); 
            ans->ad2 = ( (-1) * coef->b + sqr_discr ) / (2 * coef->a);
...
{	
    ...
    {
        return ABOUT_ZERO; // make it enum or boolean
    }
    else
    {
